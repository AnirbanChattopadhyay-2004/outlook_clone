...
const Page1 = () => {
  ...
  useEffect(() => {
    async function getMails() {
      const a = localStorage.getItem("unreadmailspage1");
      const b = localStorage.getItem("readmailspage1");
      const c = localStorage.getItem("favoritemailspage1");
      if (a) {
  ...
  useEffect(() => {
    if (readmails.length > 0)
      localStorage.setItem("readmailspage1", JSON.stringify(readmails));
    if (unreadmails.length > 0)
      localStorage.setItem("unreadmailspage1", JSON.stringify(unreadmails));
  ...
  if (!unreadmails) return <div className="h-screen text-black bg-transparent">Loding...</div>;
  return (
    <main className="px-5">
      ...
      <section className={`flex  gap-2 h-[100%] `}>
        ...
        {selectedmail && (<section className="w-[55vw] h-[85vh]   flex justify-start sticky top-[12vh] border rounded-md border-[#CFD2DC]">
            <Mailbody
              mailbody={mailbody}
